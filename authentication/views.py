...
from common.utils import email_verification_token, generate_verification_url, send_activation_email
...
class UserRegistrationViewset(ModelViewSet):
    ...
    def register_user(self, request):
        ...
        if not serializer.is_valid():
            return Response(
                {'success': True, 'msg': serializer.errors}, status=status.HTTP_400_BAD_REQUEST
            )
...
class EmailVerificationViewset(GenericAPIView):
    def get(self, request, uuid, token):
        try:
            ...
            return Response({'msg': "Error verifying email"}, status=status.HTTP_400_BAD_REQUEST)
...
class LoginViewset(ViewSet):
    ...
    def perform_login(self, request):
        ...
        if not serialiser.is_valid():
            return Response({"success": False, "error": serialiser.errors, "token": ""})

        token = serialiser.data.get('token')
        if not token:
            return Response(
                {"msg": "Invalid username or password", "token": None},
                status=status.HTTP_401_UNAUTHORIZED,
            )

        return Response(
            {"msg": "Successfully logged in", "token": token}, status=status.HTTP_200_OK
        )
...
class AccountDeactivationViewset(ViewSet):
    ...
    def deactivate_account(self, request):
        ...
        return Response(
            {'success': True, 'msg': 'Account deactivated'}, status=status.HTTP_202_ACCEPTED
        )
...
class RequestPasswordResetAPIView(GenericAPIView):
    ...
    def post(self, request):
        ...
        return Response(
            {'msg': 'Email sent. Please check your inbox for a password reset email.'},
