...
def generate_verification_url(user, request):
    ...
    email_verification_url = reverse('user-auth:email-verification', args=[uuid, token])
    url = request.build_absolute_uri(email_verification_url)
    username = user.email.split("@")[0]
    return {'url': url, 'username': username, 'email': user.email}
...
def send_activation_email(url=None, username=None, email=None):
    ...
    # username = user.email.split("@")[0]
    context = {'username': username, 'url': url, 'app_name': "Todos Manager"}
